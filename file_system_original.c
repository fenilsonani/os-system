...
int delete_file(const char* name) {
    for (int i = 0; i < fs.file_count; i++) {
        if (!fs.file_pool[i].is_deleted && strcmp(fs.file_pool[i].filename, name) == 0) {
            fs.file_pool[i].is_deleted = 1;
            // Clear data to simulate deletion more accurately (optional)
            memset(fs.file_pool[i].data, 0, MAX_FILESIZE);
            fs.file_pool[i].size = 0;
...
void list_files() {
    ...
    for (int i = 0; i < fs.file_count; i++) {
        if (!fs.file_pool[i].is_deleted) {
            printf("%-20s %-10d\n", fs.file_pool[i].filename, fs.file_pool[i].size);
            count++;
        }
    }
